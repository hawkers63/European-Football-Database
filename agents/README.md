...
graph TD
    ...
    DB --> Parser["2b. modern_era_parser.md (Group Stage and Swiss Phase)"]
    Parser --> Stats["2c. stats_analyst.md (H2H, Goals, Leaderboards)"]
    Stats --> UI["3. ui_ux_developer.md (Modernise UI and Brackets)"]
    UI --> CICD["ci_cd_guardian.md (Actions, PR gates)"]
...
1. **Author briefs**: Add and maintain role files in [`agents/`](./), including [`modern_era_parser.md`](modern_era_parser.md), [`ci_cd_guardian.md`](ci_cd_guardian.md), and [`stats_analyst.md`](stats_analyst.md).
2. **Author workflows**: Keep [`.github/workflows/verify_database.yml`](../.github/workflows/verify_database.yml) so all pushes and pull requests run `python build_database.py --force` and `pytest tests/`.
